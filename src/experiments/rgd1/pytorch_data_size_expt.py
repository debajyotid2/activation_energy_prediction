...
RESULTS_PATH = "./results_no_rev_in_test.csv"
...
def main() -> None:
    ...
    results = pd.DataFrame(columns=["Dataset size", "MAE train", "MAE test",
                                    "MSE train", "MSE test", 
                                    "R^2 train", "R^2 test"])

    for fac in range(30, 100, 5):
        dataset_frac = fac/100

        start_time = time.perf_counter()
        logging.info("Loading data...")
        train_loader, val_loader, test_loader = pytorch_dataset.load_dataloaders(
        ...
        dataset_size = len(y_train)+len(y_train)*int(VAL_FRAC/(1-VAL_FRAC))+len(y_test)

        results.loc[-1] = [dataset_size, mae_loss_train.detach().numpy(),\
                            mae_loss_test.detach().numpy(), 
                            mse_loss_train.detach().numpy(), 
                            mse_loss_test.detach().numpy(), 
                            r_sq_train.detach().numpy(), 
                            r_sq_test.detach().numpy()]
        results.index += 1
