...
class RGD1(torch.utils.data.Dataset):
    ...
    """
    Custom PyTorch dataset for the RGD1 reaction dataset. Original dataset authored by Qiyuan Zhao, 
    Brett Savoie, Michael Woulfe, Sai Mahit Vaddadi, Lawal A. Ogunfowora, Sanjay Garimella.
    """
    def __init__(self, 
    ...
    def _load_compressed_dataset(self,
        ...
        if not all([features_save_path.exists(), targets_save_path.exists()]):
            ...
            return None, None
        return npz_ops.load_from_npz(features_save_path), npz_ops.load_from_npz(targets_save_path)

    def __getitem__(self, idx: int | list[int]) -> tuple[torch.Tensor, torch.Tensor]:
        X = torch.from_numpy(self._X[idx, :])
        Y = torch.tensor(self._Y[idx])
...
def load_data(download_dirpath: Path,
              radius: int,
              n_bits: int, 
              batch_size: int = 32,
              val_frac: float = 0.1,
